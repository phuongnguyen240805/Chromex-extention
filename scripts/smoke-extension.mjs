...
try {
  ...
  await page.locator("#ladipage-app-grid-toggle").click();
  const ladipageAppGridState = await page.evaluate(() => ({
    gridVisible: Boolean(document.querySelector(".ladipage-app-grid-popover")),
    facebookAdsTileVisible: Boolean(
      document.querySelector('[data-ladipage-app-id="facebook-ads"]'),
    ),
  }));
  if (!ladipageAppGridState.gridVisible || !ladipageAppGridState.facebookAdsTileVisible) {
    throw new Error(
      `Smoke test failed: Ladipage app grid is incomplete (${JSON.stringify(ladipageAppGridState)}).`,
    );
  }
  ...
  if (
    ...
    throw new Error(
      `Smoke test failed: Facebook Ads embed is incomplete (${JSON.stringify(ladipageEmbeddedAppState)}).`,
    );
  }
  await page.locator("#close-ladipage-app").click();
  await waitForSmokeComposer(page);
