(async () => {
  ...
  const { hookFetch } = await import("../../../contents/core/ajax-hook");
  const {
    notify,
    downloadData,
    closest,
    ...
    injectCssCode,
    getFBAIODashboard
  } = await import("../../../contents/core/helper");

  const videoById = new Map();

  // reference to Cached
  (window as any).fbaio_tiktok_addDownloadVideoBtn = () => videoById;
  ...
  hookFetch({
    onAfter: async (url: any, options: any, response: any) => {
  ...
  function hasDownloadBtn(ele: Element, vidId?: string) {
    const target = ele.querySelector(`.${id}`) as HTMLElement;
    return target && (!vidId || target.dataset.id === vidId);
  }
  function removeDownloadBtn(ele: Element) {
  ...
  setInterval(() => {
    ...
    for (const item of [...explore_videos, ...search_videos]) {
      if (!hasDownloadBtn(item)) {
        const vidId = (item
          .querySelector('a[href*="tiktok.com"][href*="/video/"]') as HTMLAnchorElement)
          ?.href?.split("video/")
          ?.at(-1);
        if (vidId && videoById.has(vidId)) {
    ...
    for (const vid of you_may_like_vids) {
      if (!hasDownloadBtn(vid)) {
        const vidId = (vid
          .querySelector('a[href*="/video/"]') as HTMLAnchorElement)
          ?.href?.split("video/")
          ?.at(-1);
        ...
        if (vidId && videoById.has(vidId)) {
          ...
          (vid as HTMLElement).style.position = "relative";
          vid.appendChild(btn);
        }
      }
    }
    ...
    const interval = setInterval(() => {
      if ((window as any)?.__UNIVERSAL_DATA_FOR_REHYDRATION__) {
        const data = JSON.parse(
          (window as any).__UNIVERSAL_DATA_FOR_REHYDRATION__.textContent
        );
        const vidData =
