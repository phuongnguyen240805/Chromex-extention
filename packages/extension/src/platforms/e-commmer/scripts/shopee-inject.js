  ...
  if (isTargetApi) {
    ...
    response
      ...
      .then((data) => {
        ...
        saveData("shopee_product_detail", data);
      });
  }
...
function saveData(task, json) {
  switch (task) {
    case "shopee_product_detail":
      console.log("GET ITEM", json);
      const { item_id: t, shop_id: e } = json.data.item;
      if (json.data.item.item_id) {
        const i = json.data;
        // sessionStorage.setItem(`sx-product-info-${t}-${e}`, JSON.stringify(i));
        const channel = new BroadcastChannel("sx-crx-channel");
        channel.postMessage({
          type: "sx-product-shopee",
          data: {
            id: `${t}-${e}`,
