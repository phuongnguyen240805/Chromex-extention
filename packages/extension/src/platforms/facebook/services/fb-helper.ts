import { deepFind } from "../../../contents/core/helper";

export function wrapGraphQlParams(params: any) {
  const formBody = [];
...
export async function fetchGraphQl(params: any, fb_dtsg?: string) {
  let form;
...
export async function getFbdtsg() {
  let methods = [
    () => (window as any).require?.("DTSGInitData")?.token,
    () => (window as any).require?.("DTSG")?.getToken(),
    () => {
    ...
    () => (window as any).require?.("DTSG_ASYNC")?.getToken(),
  ];
...
export async function getEntityAbout(entityID: string, context: string = "DEFAULT") {
