...
import { shouldInterceptComposerDropdownOnEnter, shouldSubmitComposerOnKeydown } from "./composer-submit";
import { createSubmittedComposerFileAttachmentState } from "./composer-attachment-submit";
import { createPendingComposerDraftState, createRestoredComposerDraftState } from "./composer-draft";
import { getDroppedFiles, hasComposerDropPayload } from "./composer-drop";
import { isTextFirstActionCard } from "./action-card-routing";
import { mergePromptResultActionCards } from "./action-card-state";
import {
...
} from "./conference-mode";
import { buildComicImagePrompt } from "../comic-image-prompt";
import { normalizeSelectedTextContext } from "../page-selection-context";
import { calculateComposerTextareaAutosize } from "./composer-textarea-autosize";
import { canSendComposerMessage } from "./composer-send-guard";
import { parseGoalCommand } from "./goal-command";
import {
  didComposerPrimaryActionChangeForDraftInput,
  resolveComposerPrimaryAction,
} from "./composer-primary-action";
import { shouldSendComposerAsTurnSteer } from "./turn-steer-routing";
import { resolveAcceptedPromptActiveTurn } from "./active-turn-after-send";
import {
  clearTransientComposerCommandPills,
  removeComposerCommandPill,
  upsertComposerCommandPill,
  type ComposerCommandPill,
} from "./composer-command-pills";
import { createDebouncedTask } from "./debounced-task";
import { prepareMessageReplay } from "./message-actions";
import { createConversationMessageAttachments } from "./message-attachments";
import { isSafeMessageImageUrl, renderMessageContentHtml } from "./message-content";
import { createExternalImagePreviewUrl } from "./external-image-preview";
import {
...
} from "./profile-question";
import { renderPendingProfileQuestionCard } from "./profile-question-card";
import {
...
import { createRenderBatcher } from "./render-batcher";
import {
...
} from "./prompt-activity-lifecycle";
import { collectBusyConversationIds } from "./busy-conversations";
import {
  getPermissionRequestForMessage,
  getPermissionRequestForRuntimeResponse,
  type PermissionRequestPlan,
} from "../permission-plans";
import {
  extractSlashQuery,
...
import { createOnlineImagePromptExtractionPrompt } from "./online-image-prompt";
import {
...
} from "./image-attachment-suggestions";
import { parseVoiceNavigationCommand } from "./voice-commands";
import { shouldAutoReconnectVoice } from "./voice-reconnect";
import { shouldInterruptVoiceOutputForTranscript } from "./voice-barge-in";
import { listChangedSpeechRecognitionResults, type SpeechRecognitionResultEventLike } from "./voice-recognition-results";
import {
...
import { buildRealtimeVoiceStartMessage, listCodexRealtimeVoiceOptions } from "./voice-session";
import { createRealtimeVoiceContextAppendText } from "./voice-turn-context";
import { shouldRouteRealtimeVoiceTranscriptThroughPrompt } from "./voice-agentic-routing";
import { extractRealtimeVoiceHandoffPrompt } from "./voice-handoff";
import { createVoiceRoutePreviewPayload as createSanitizedVoiceRoutePreviewPayload } from "./voice-route-preview";
import {
...
} from "./voice-live-captions";
import { renderVoiceMessageIcon } from "./voice-message-icons";
import { shouldShowPermissionStatusBanner } from "./permission-status";
import { createConfirmedHistorySearchRequest } from "./history-search";
import { requestOptionalPermissionsWithResult } from "./permission-request";
import { resolvePermissionRetryPrompt } from "./permission-retry";
import { formatCurrentTabReferenceLabel, formatTabReferenceLabel, getTabReferenceInitial } from "./tab-reference";
import { formatTabMentionUrl, listTabMentionOptions, toggleSelectedTabId } from "./tab-mentions";
import { listSettingsSections } from "./settings-panel";
import { DEFAULT_PROFILE_ICON_ID, renderProfileIcon } from "./profile-icons";
import { renderMessageActionIcon } from "./message-action-icons";
import { shouldRenderAssistantMessageActions } from "./message-action-visibility";
import {
  clampSlashCommandIndex,
  getNextSlashCommandIndex,
  isSlashCommandArrowKey,
} from "./slash-navigation";
import {
  DEFAULT_PROFILE_ID,
  normalizeSelectedProfileIdForProfiles,
  resolveComposerProfileSelection,
} from "./profile-selection";
import { localizeBuiltinProfiles } from "../profile-localization";
import { shouldOfferApiKeyFallbackForError } from "./oauth-fallback";
import {
...
import {
  ...
  LADIPAGE_EMBEDDED_APPS,
  type LadipageAppId,
} from "./ladipage-apps";
import { createAssistantFailureMessage } from "./submission-failure";
import { isBridgeImageAssetRef, resolveImagePreviewRefForUi } from "./image-preview-assets";
import {
...
} from "./generated-image-attachments";
import {
  isPendingImageMessage,
  shouldRenderConversationMessage,
} from "./conversation-message-visibility";
import {
  formatTraceDetail,
  formatTraceSummary,
  formatTraceTitle,
...
} from "./message-trace-formatting";
import { isQuickInteractionLocked } from "./interaction-lock";
import {
...
import { isYouTubeCurrentMomentAction, type YouTubeCurrentMomentPromptResult } from "../youtube-current-moment";
...
console.log("Sidepanel script starting...");
let rootElement = document.querySelector<HTMLDivElement>("#app");
if (!rootElement) {
  console.log("Creating #app root element...");
  rootElement = document.createElement("div");
  rootElement.id = "app";
  document.body.appendChild(rootElement);
} else {
  console.log("Found existing #app root element.");
}
const root: HTMLDivElement = rootElement;
root.addEventListener("click", handleYouTubeTimestampClick);
console.log("Sidepanel root initialized.");
...
let composerVoiceInputAudioData: Uint8Array | null = null;
...
let ladipageAppGridOpen = false;
let activeLadipageAppId: LadipageAppId | null = null;
...
async function initialize(options: { forceCatalog?: boolean } = {}): Promise<void> {
  ...
  console.log("Sidepanel initialize starting, sending ui.init to background...");
  ...
  initPayloadPromise.then(p => console.log("ui.init response received:", p))
...
function scheduleInitialize(options: { forceCatalog?: boolean } = {}): Promise<void> {
  console.log("scheduleInitialize called", options);
...
function renderNow(): void {
  ...
  const isEmptyChat = activeLadipageAppId === null && state.activeView === "chat" && state.messages.length === 0;
    ...
    <div class="shell ${isEmptyChat ? "shell-empty-chat" : ""} ${showOnboarding ? "shell-onboarding" : ""}">
      ${
        ...
        <div class="top-actions">
          ...
          <button
            id="ladipage-app-grid-toggle"
            class="icon-button ladipage-app-grid-toggle ${ladipageAppGridOpen ? "active" : ""}"
            title="Ứng dụng Ladipage"
            aria-label="Ứng dụng Ladipage"
            aria-haspopup="menu"
            aria-expanded="${ladipageAppGridOpen ? "true" : "false"}"
          >${renderUiIcon("layout-grid")}</button>
          ${ladipageAppGridOpen ? renderLadipageAppGrid() : ""}
          <button
            id="app-menu-toggle"
      ...
      ${
        showOnboarding || activeLadipageAppId
          ? ""
...
function isInsideFloatingSurfaceInteraction(target: Element): boolean {
  ...
  if (
    ladipageAppGridOpen &&
    target.closest("#ladipage-app-grid-toggle, .ladipage-app-grid-popover")
  ) {
    return true;
  }
...
function renderLadipageAppGrid(): string {
  return `
    <section class="ladipage-app-grid-popover" role="menu" aria-label="Ứng dụng Ladipage">
      <div class="ladipage-app-grid-header">
        <div>
          <p class="ladipage-app-grid-eyebrow">LADIPAGE</p>
          <h2>Kho ứng dụng</h2>
        </div>
        <span>${LADIPAGE_EMBEDDED_APPS.length} ứng dụng</span>
      </div>
      <div class="ladipage-app-grid">
...
function renderLadipageEmbeddedApp(): string {
  if (!activeLadipageAppId) {
    return "";
  }
...
function installSmokeHarness(): void {
  ...
  window.__CODEX_SIDEPANEL_SMOKE__ = {
    ...
    inspectCommandPopoverForTest(text: string) {
      ...
      const builtinSmokeProfiles = localizeBuiltinProfiles(listProfileTemplates(), state.uiLocale);
      const builtinSmokeProfileIds = new Set(builtinSmokeProfiles.map((profile) => profile.id));
      state.profiles = [
        ...builtinSmokeProfiles,
        ...state.profiles.filter((profile) => !builtinSmokeProfileIds.has(profile.id)),
      ];
...
async function removeComposerCommandPillSelection(pillId: string, kind: ComposerCommandPill["kind"]): Promise<void> {
  ...
  void sendRuntimeMessage({ type: "profile.select", profileId: DEFAULT_PROFILE_ID }).catch((error) => {
    state.initError = toUserFacingRuntimeError(error, stringsForState().errors.init);
    render();
  });
  scheduleConversationPersist();
...
function bindEvents(): void {
  ...
  root.querySelector<HTMLButtonElement>("#ladipage-app-grid-toggle")?.addEventListener("click", () => {
    root.querySelector<HTMLTextAreaElement>("#composer")?.blur();
    state.appMenuOpen = false;
    state.attachmentMenuOpen = false;
    state.composerModelMenuOpen = false;
    ...
    state.mentionActiveIndex = 0;
    state.slashQuery = null;
    ladipageAppGridOpen = !ladipageAppGridOpen;
    renderSync();
  });

  root.querySelectorAll<HTMLButtonElement>("[data-ladipage-app-id]").forEach((button) => {
    button.addEventListener("click", () => {
      const appId = button.dataset.ladipageAppId as LadipageAppId | undefined;
      if (!appId) {
        return;
      }
      activeLadipageAppId = appId;
      ladipageAppGridOpen = false;
      state.appMenuOpen = false;
      renderSync();
    });
