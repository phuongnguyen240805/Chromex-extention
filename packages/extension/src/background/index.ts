...
import { createYouTubeCurrentMomentPromptResult } from "../youtube-current-moment";
import {
...
} from "../browser-permission-errors";
import {
...
import { resolveVisibleCurrentConversation } from "./conversation-history";
import { NativeBridgeClient } from "./native-bridge-client";
import { assertApiKeyLoginExplicitlyConfirmed } from "./api-key-login-guard";
import { createUserProfileTemplate, updateUserProfileTemplate } from "../profile-templates";
import { inferActionCardsForOpenTab } from "./site-suggestions";
import {
...
} from "./image-edit-timeout";
import { isRecoverableTurnSteerError } from "./turn-steer-recovery";
import {
  createVisibleTabCaptureThrottle,
  DEFAULT_VISIBLE_TAB_CAPTURE_MIN_INTERVAL_MS,
  isCaptureVisibleTabQuotaError,
} from "./visible-tab-capture-throttle";
import { buildVoiceSessionStartParams, buildVoiceSessionStopParams } from "./voice-session-routing";
import { createVoiceSessionContextPrompt } from "./voice-context";
import { buildInfographicPrompt } from "../infographic-prompt";
import { buildSlideDeckImagePrompt } from "../slide-deck-image-prompt";
import {
  shouldLogBackgroundMessageError,
  toExpectedPermissionErrorResponse,
} from "./background-error-response";
import { createPopoutUrlPath, selectBrowserWindowIdForPopout } from "./popup-window-target";
import {
...
import { resolveUiLocale } from "../ui-language";
import { getUiStrings } from "../sidepanel/i18n";
import { selectEditablePageImageCandidate, type EditablePageImageCandidate } from "../page-image-target";
import {
...
import type { SkillOption } from "../sidepanel/skills";
import type { PromptActivityPhase } from "../sidepanel/prompt-activity";
import { mergeStructuredInputsWithEnabledCodexSkills } from "../codex-skill-settings";
import { requiresPluginCompanionAppConnection } from "../plugin-connection-availability";
import {
...
} from "./route-structured-inputs";
import { isRecoverableMissingCodexThreadError, shouldAutoCompactConversation } from "./auto-compact";
import { resolveBridgeEventConversationId } from "./bridge-event-routing";
import { ConversationRuntimeRegistry } from "./conversation-runtime";
import {
  buildDeferredBrowserActionMessage,
  extractDeferredBrowserActionText,
  shouldResumeDeferredBrowserDomAction,
} from "./deferred-browser-action";
import {
...
} from "./pdf-page-context";

import { bootIntegratedPlatforms } from "../core/startup";

// Khởi động trạm quản lý module nền tảng độc lập
bootIntegratedPlatforms();
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  // Bỏ qua các gói tin giao tiếp nội bộ của nền tảng e-commerce (đặc trưng bởi thuộc tính func)
  // để tránh việc luồng core đóng kênh phản hồi (sendResponse) trước khi module e-commerce kịp xử lý.
  if (message && typeof message.func === "string") {
    return false;
  }

  // Bỏ qua các gói tin giao tiếp nội bộ của nền tảng keywordseverywhere (đặc trưng bởi thuộc tính cmd)
  if (message && typeof message.cmd === "string" && message.cmd !== "open-side-panel") {
    return false;
  }
  ...
  if (message.cmd === "open-side-panel") {
    const fallbackToPopup = () => {
      console.warn("Falling back to popOutChat() because sidePanel.open was blocked.");
      popOutChat().catch(console.error);
    };

    try {
      if (sender.tab?.windowId) {
        chrome.sidePanel.open({ windowId: sender.tab.windowId }).catch(err => {
          console.error("SidePanel open error (windowId):", err);
          fallbackToPopup();
        });
      } else if (sender.tab?.id) {
        chrome.sidePanel.open({ tabId: sender.tab.id }).catch(err => {
          console.error("SidePanel open error (tabId):", err);
          fallbackToPopup();
        });
      } else {
        fallbackToPopup();
      ...
      console.error("SidePanel sync error:", e);
      fallbackToPopup();
...
/**
 * Dynamic Anti-Phishing Rules Management
 * Syncs declarativeNetRequest rules with the user's antiPhishing setting.
 */
...
const syncPhishingRules = (antiPhishing: boolean) => {
  if (antiPhishing) {
    ...
    }).catch(err => console.error("Failed to enable phishing rules:", err));
  } else {
    chrome.declarativeNetRequest.updateEnabledRulesets({
      disableRulesetIds: PHISHING_RULESETS
    }).catch(err => console.error("Failed to disable phishing rules:", err));
  }
};

// Listen for storage changes from the UI (Zustand store)
chrome.storage.onChanged.addListener((changes, area) => {
  if (area === "local" && changes["wsk-state"]) {
    try {
      ...
      console.error("Failed to parse wsk-state on change:", err);
    }
  }
});
...
chrome.storage.local.get("wsk-state", (data) => {
  try {
    ...
    console.error("Failed to parse wsk-state on startup:", err);
  }
});
